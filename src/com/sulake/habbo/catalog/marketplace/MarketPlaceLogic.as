package com.sulake.habbo.catalog.marketplace
{
   ...
   public class MarketPlaceLogic implements IMarketPlace
   {
      ...
      private var var_2240:MarketplaceItemStats;
      ...
      {
         if(!var_163 || !_catalog || !_catalog.getPurse())
         {
            return;
         }
         var _loc2_:MarketPlaceOfferData = var_163.getValue(param1) as MarketPlaceOfferData;
         if(!_loc2_)
         {
            return;
         }
      ...
      {
         if(param1.furniCategoryId != var_2140 || param1.furniTypeId != var_2139)
         {
            return;
         }
         var_2240 = param1;
      ...
      public function onBuyResult(param1:IMessageEvent) : void
      {
         ...
         {
            item = var_163.remove(parser.requestedOfferId);
            if(item != null)
            {
               item.dispose();
            }
         ...
         {
            updateItem = var_163.getValue(parser.requestedOfferId) as MarketPlaceOfferData;
            if(updateItem)
            {
               updateItem.offerId = parser.offerId;
               updateItem.price = parser.newPrice;
               --updateItem.offerCount;
               var_163.add(parser.offerId,updateItem);
            }
            var_163.remove(parser.requestedOfferId);
            showConfirmation(const_1538,updateItem);
            _visualization.listUpdatedNotify();
         }
      ...
      public function redeemSoldOffers() : void
      {
         ...
         for each(_loc2_ in _loc1_)
         {
            ...
            if(_loc3_.status == MarketPlaceOfferState.const_401)
            {
      ...
      public function onOwnOffers(param1:IMessageEvent) : void
      {
         ...
         var_2057 = _loc3_.creditsWaiting;
         var_206 = new Map();
      ...
      public function onOffers(param1:IMessageEvent) : void
      {
         ...
         for each(_loc4_ in _loc3_.offers)
         {
            ...
            _loc6_.timeLeftMinutes = _loc4_.timeLeftMinutes;
            var_163.add(_loc4_.offerId,_loc6_);
         }
         if(_visualization == null)
         {
            return;
      ...
      {
         requestOffers(var_2243,var_2241,var_2103,var_2242);
      }
