package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IAvatarImageListener
   {
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(_selectedColorIndex >= var_325.length)
         {
            return "";
         }
         var petColor:IPetColor = var_325[_selectedColorIndex] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:IBreed = var_654[var_1654] as IBreed;
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         if(_selectedColorIndex < 0 || _selectedColorIndex > var_325.length)
         {
      ...
      override public function init() : void
      {
         ...
         _loc2_.addEventListener(WindowEvent.const_122,onTextWindowEvent);
         var_640 = new Map();
         ...
         var_963 = getPetTypeIndexFromProduct(_loc5_.localizationId);
         var_325 = [];
         var_654 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc11_ = _loc10_.avatarRenderManager.petDataManager.getPetData(var_963,this);
               if(_loc11_)
               {
                  var_654 = _loc11_.breeds;
                  var_325 = _loc11_.colors;
                  if(var_325 == null)
                  {
                     var_325 = [];
                  }
         ...
         var _loc6_:int = 0;
         while(_loc6_ < var_654.length)
         {
            _loc12_ = var_654[_loc6_] as IBreed;
            _loc3_.push(_loc4_.getKey(getRaceLocalizationKey(var_963,_loc12_.id),getRaceLocalizationKey(var_963,_loc12_.id)));
            _loc6_++;
         }
         var_640.add(var_963,_loc5_);
         ...
         events.addEventListener(WidgetEvent.const_1019,onApproveNameResult);
         _loc5_ = var_640.getWithIndex(0);
         if(_loc5_ != null)
         {
            events.dispatchEvent(new SelectProductEvent(_loc5_));
         }
         var _loc8_:* = [];
         for each(_loc9_ in var_325)
         {
      ...
      private function onApproveNameResult(param1:CatalogWidgetApproveNameResultEvent) : void
      {
         ...
         (page.viewer.catalog as HabboCatalog).showPurchaseConfirmation(var_640.getWithIndex(0),page,p);
      }
      ...
      {
         var _loc7_:* = null;
         var _loc1_:Offer = var_640.getWithIndex(0);
         if(_loc1_ == null)
         {
            return;
         }
         ...
         if(_selectedColorIndex >= 0 && _selectedColorIndex < var_325.length)
         {
            _loc7_ = var_325[_selectedColorIndex] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:IBreed = var_654[var_1654] as IBreed;
         if(!_loc3_)
         {
            return;
         }
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(var_963,_loc3_.id,_loc2_,AvatarScaleType.const_58,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_285,AvatarAction.const_669);
