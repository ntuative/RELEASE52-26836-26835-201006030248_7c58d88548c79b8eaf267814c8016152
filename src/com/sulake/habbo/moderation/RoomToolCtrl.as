package com.sulake.habbo.moderation
{
   ...
   public class RoomToolCtrl implements IDisposable, TrackedWindow
   {
      ...
      private function onChatlog(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         _main.windowTracker.show(new ChatlogCtrl(new GetRoomChatlogMessageComposer(0,_data.flatId),_main,WindowTracker.const_712,_data.flatId),var_53,false,false,true);
      }
      ...
      private function act(param1:Boolean) : void
      {
         ...
         var _loc2_:int = determineAction(param1,var_756.isSelected);
         _main.connection.send(new ModeratorActionMessageComposer(ModeratorActionMessageComposer.const_1297,_loc2_,var_400.text,"",""));
         if(false)
         {
            _main.connection.send(new ModerateRoomMessageComposer(_data.flatId,var_1095.isSelected,var_1096.isSelected,var_756.isSelected));
         }
         this.dispose();
      }
      ...
      private function determineAction(param1:Boolean, param2:Boolean) : int
      {
         if(param2)
         {
            return !!param1 ? 0 : int(ModeratorActionMessageComposer.const_1181);
         }
         return !!param1 ? 0 : int(ModeratorActionMessageComposer.const_1328);
      }
      ...
      public function populate() : void
      {
         ...
         var_756 = ICheckBoxWindow(var_53.findChildByName("kick_check"));
         var_1095 = ICheckBoxWindow(var_53.findChildByName("lock_check"));
         var_1096 = ICheckBoxWindow(var_53.findChildByName("changename_check"));
